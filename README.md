# João Pedro Scarlateli

Transitioning Software Engineer | Backend Development Focus  
7+ years in ERP/EMR systems (Philips & Healthcare IT)  
Strong experience in SQL & Oracle | Expanding into Computer Science & Backend Development (Harvard CS50, MITx, Codecademy)  

## LeetCode

![LeetCode Stats](https://leetcard.jacoblin.cool/Scarlateli?ext=heatmap&theme=dark)


### Tecnologias  e Ferramentas

![SQL](https://img.shields.io/badge/SQL-336791?style=for-the-badge&logo=postgresql&logoColor=white)  
![Oracle](https://img.shields.io/badge/Oracle-F80000?style=for-the-badge&logo=oracle&logoColor=white)  
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)  
![C](https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white)  
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)  
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)  
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white)  
![Warp Terminal](https://img.shields.io/badge/Terminal-Warp-4D4D4D?style=for-the-badge&logo=gnu-bash&logoColor=white)  
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)  
---

## Stack atual

- **Programming & Databases:** SQL (Oracle), Python (beginner), C (basic)  
- **Software Development:** Git/GitHub, Debugging, Troubleshooting  
- **API & Cloud (intro):** REST (Postman), AWS Fundamentals, Oracle OCI  
- **Tools:** VS Code, Jira, Docker (basic), WARP Terminal  

---

## Organização dos projetos


Repositórios organizados com foco em:  
- Exercícios e problem sets de Ciência da Computação (Harvard CS50, MITx).  
- Prática de algoritmos e estruturas de dados (LeetCode, HackerRank).  
- SQL para análise e manipulação de dados.  
- Scripts e automações em Python.  
- Testes e consumo de APIs RESTful.  
- Workflows modernos com Git e versionamento no GitHub.  

---

## Áreas de interesse

- Backend Development (SQL, APIs, Systems Integration)  
- Computer Science (Algorithms, Data Structures, OOP)  
- Problem Solving (LeetCode, HackerRank, CS50/MITx)   

---

## Contato

[LinkedIn](https://www.linkedin.com/in/jp-scarlateli/)
[E-mail](mailto:jprebello10@gmail.com)

---


